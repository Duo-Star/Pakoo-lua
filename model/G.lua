...
G={
  ...
  __index = {
    ...
    function init(g,todo)
      ...
      .setTextSize(50)
      --网格画笔
      g.paint.grid=Paint().setColor(0x99000000)
      .setStyle(Paint.Style.STROKE)
      .setStrokeWidth(8)
      .setAntiAlias(true)
      .setStrokeCap(Paint.Cap.ROUND)
      ...
      holder.addCallback(SurfaceHolder.Callback {
        surfaceChanged = function(holder, format, width, height)
          if g.animation.isPaused() then
            g.animation.start()
          end
        end,
        surfaceCreated = function(holder)
          g.animation.addUpdateListener(ValueAnimator.AnimatorUpdateListener {
    ...
    drawPoint=function(g,v,p_)
      g.canvas.drawCircle(v.x*g.lam+g.o.x, v.y*(-g.lam)+g.o.y, 6, p_ )--or g.paint.a)--以后不管画画不带笔的那些人了
    end,
    drawBitmap=function(g,bitmap,v,s,p)
      local scale=s*g.lam
      local m=Matrix()
      m.reset()
      m.postScale(scale,scale)
      m.postTranslate(v.x*g.lam+g.o.x, v.y*(-g.lam)+g.o.y)
      g.canvas.drawBitmap(bitmap,m,p)
    end,
