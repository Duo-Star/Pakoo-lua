...
Piglin={
  ...
  __index = {
    new=function(data)
      ...
      return setmetatable(data,Piglin)
    end,
    ...
    ptOutline=Paint().setColor(0xFF795548).setStrokeWidth(1) .setStyle(Paint.Style.FILL),
    onDraw=function(String_,graph)
      ...
      graph:drawBitmap(res.bitmap.PiglinFace,String_.p+Vector(-.43,.35),0.011,ptOutline)
    end,
    onCreate=function(torch,pakoo)

    end,
    onTurnOn=function(cp)
      soundPool.play(res.sound.Piglin_idle1, 1,1,0,0,1)
    end,
...
onDelete=function(item,map)
    ...
    isThis=function(sth)
      return getmetatable(sth)==Piglin
    end,
